...
const PluginRenderer = ({ plugins }) => {
    ...
    const loadPluginWithRetry = (
        ...
        const attemptLoad = (remainingRetries) => {
            if (window.pluginInstances && window.pluginInstances[pluginType]) {
                ...
                window.pluginInstances[pluginType].initializePluginUI(
                    store,
                    instanceId,
                    `plugin-container-${index}`,
                    injectReducer,
                    wssInstance,
                    Server_Addr,
                    axiosInstance,
                ...
                if (remainingRetries > 0) {
                    console.error(
                        `initializePluginUI not found for ${instanceId}. Retrying in ${delay / 1000} seconds...`,
                    );
                    setTimeout(() => attemptLoad(remainingRetries - 1), delay);
                } else {
                    console.error(
                        `Failed to load plugin ${instanceId} after multiple attempts.`,
                    );
                }
            }
        };
    ...
    useEffect(() => {
        ...
        try {
            pluginContainerData =
                document.getElementById("plugin-container-0").textContent;
        } catch (err) {
    ...
    return <>{/* You can return null or some loading UI here if needed */}</>;
};
